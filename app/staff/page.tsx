...
export default async function StaffPage() {
  ...
  // 1. Editorial Board: Authors with a defined position or role
  const editorialBoard = authors.filter((a: any) => a.position || a.role);

  // 2. Contributors: Authors without a position or role
  const contributors = authors.filter((a: any) => !(a.position || a.role));
  ...
  return (
    <main className="min-h-screen bg-white font-sans">
      <div className="max-w-4xl mx-auto p-8 py-16">
        ...
        {/* PROFILE CARDS: Editorial Board with positions */}
        {editorialBoard.length > 0 && (
          <div className="flex flex-col gap-8 mb-16">
            {editorialBoard.map((author: any) => (
              <Link
                key={author._id}
                href={author.slug ? `/author/${author.slug}` : "#"}
                className="group flex flex-col md:flex-row items-center md:items-start gap-8 p-6 border border-gray-100 rounded-xl hover:shadow-xl transition bg-gray-50"
              >
                {author.image ? (
                  <img
                    src={urlFor(author.image).width(200).height(200).url()}
                    alt={author.name}
                    className="w-32 h-32 md:w-40 md:h-40 rounded-full object-cover shadow-md border-4 border-[#FFD700] group-hover:scale-105 transition flex-shrink-0"
                  />
                ) : (
                  ...
                  {author.categories && author.categories.length > 0 && (
                    <div className="flex flex-wrap gap-2 justify-center md:justify-start mb-4">
                      {author.categories.map((cat: any) => (
                        <span
                          key={typeof cat === "string" ? cat : cat._id || cat.title}
                          className="bg-white border border-[#FFD700] text-[#800000] text-xs font-bold uppercase tracking-wider py-1 px-3 rounded-full"
                        >
